...
class Api {
  ...
  getIpsecP1(): Promise<any> {
   ...
   getBackup(filepath: string, ) {
    return _download("monitor/system/config/backup?scope=global", filepath, this.conn);
  }
   ...
   getCapture(mkey: string, filename: string, ): Promise<any> {
    return _download(`monitor/system/sniffer/download/${mkey}`, filename, this.conn);
  }
}
...
async function _get(url : string, conn : apinterface) {
    try {
        const {data, status} = await axios.get(`https://${
            conn.endpoint
        }/conn/v2/${url}`, {
            headers: {
...
async function _download(url: string, filename: string , conn: apinterface) {
  ...
  return new Promise((resolve, reject) => {
    https.get(`https://${conn.endpoint}/conn/v2/${url}`, { headers: { 'Authorization': `Bearer ${conn.token}` } }, function (response) {
      if (response.statusCode !== 200) {
        reject(new Error(`Something went wrong while downloading https://${conn.endpoint}/conn/v2/${url}`));
      }
